...
import openfed
# 我们在这里使用FedAddr做封装，为了方便用户调用。
# 但是在federated_c10d中，我们不这么使用。
# 因为其设计更多基础操作，封装反而不灵活。
from openfed import PACKAGES, FedAddr
from openfed.utils.safe_exited import safe_exited

from .core.federated_c10d import FederatedWorld, ProcessGroup

# 维护一个字典，去保存pg到federated_world的映射关系。
# 我们遍历的是pg，而不是federated_world。另外，这个列表只允许本模块修改和访问，故以双下划线开头。
__pg_to_federated_world_mapping: Dict[ProcessGroup,
                                      List[Informer, FederatedWorld]] = OrderedDict()

# 记录当前上层正在处理的pg是哪一个
_NULL_GP = None
__current_pg: ProcessGroup = _NULL_GP

# 我们并不希望这个参数被轻易修改，所以将它定义在这里，而不是CONSTANT里面。
_SLEEPTIME = 1.0  # seconds
...
class Joint(Thread):
    ...
    def __init__(self, fed_addr: FedAddr):
        super().__init__()
    ...
    def run(self):
        ...
        for sub_pg in sub_pg_list:
            # 为每一个pg都创建一个informer，并记录下来
            informer = Informer(fed_world.get_store(sub_pg))
            __pg_to_federated_world_mapping[sub_pg] = [informer, fed_world]
...
class Maintainer(Thread):
    ...
    def __init__(self, fed_addr: Union[FedAddr, List[FedAddr]] = None, fed_addr_file: str = None):
        """
            在客户端，只允许指定一个地址。如果多余一个地址，则会报错。
        """
        super().__init__()
        self.stopped = False
    ...
    def run(self):
        while not self.stopped and openfed.ALIVE:
            # 读取文件，自动添加
...
class Destroy(object):
    ...
    """
    @classmethod
    def destroy(cls, pg: ProcessGroup):

        # 如果删除的是当前的pg，不要忘了重置
        global __current_pg
        if pg == __current_pg:
            __current_pg = _NULL_GP

        # 获取对应的federated_world
        informer, fed_world = __pg_to_federated_world_mapping[pg]

        # 将informer状态设置成OFFINE
        informer.set_state(openfed.STATUS.OFFINE)

        # 将键值对从全局字典中移除
        del __pg_to_federated_world_mapping[pg]
        ...
        if not fed_world.is_initialized():
            ...
            # 因此可以直接删除
            federated_c10d.register.deleted_federated_world(fed_world)
    ...
    @classmethod
    def destroy_current(cls):
        global __current_pg
        if __current_pg is not _NULL_GP:
            cls.destroy(__current_pg)

    @classmethod
    def destroy_all(cls):
        for pg in __pg_to_federated_world_mapping.keys():
            cls.destroy(pg)
...
class Delivery(object):
    ...
    """
    @classmethod
    def pull_from_king(cls, pg: ProcessGroup = None) -> PACKAGES:
        """只允许在客户端调用拉取数据。
        如果不指定pg，则使用默认pg进行操作
        """
        assert openfed.is_queen()
        if pg is None:
            pg = __current_pg
        # get fed_world
        _, fed_world = __pg_to_federated_world_mapping[pg]

        # 确保是一个P2P的PG
        assert fed_world._get_group_size(
            pg) == 2, "Only P2P process group is supported"

        received = [None, None]
        # 拉取数据到本地，rank=1
        federated_c10d.gather_object(
            None, received, dst=1, group=pg, federated_world=fed_world)

        return received[1]

    @classmethod
    def push_to_king(cls, packages: PACKAGES, pg: ProcessGroup = None) -> None:
        """只允许在客户端调用，推送数据
        """
        assert openfed.is_queen()
        if pg is None:
        ...
        # 推送数据到服务器，rank=0
        federated_c10d.gather_object(
            packages, [None, None], dst=0, group=pg, federated_world=fed_world)

    @classmethod
    def pull_from_queen(cls, pg: ProcessGroup = None) -> PACKAGES:
        """只允许在服务器端调用拉取数据
        """
        assert openfed.is_king()
        if pg is None:
            pg = __current_pg
        # get fed_world
        _, fed_world = __pg_to_federated_world_mapping[pg]
        ...
        # 接收数据到本地，rank=0
        federated_c10d.gather_object(
            None, received, dst=0, group=pg, federated_world=fed_world)

        return received[0]

    @classmethod
    def push_to_queen(cls, packages: PACKAGES, pg: ProcessGroup = None) -> None:
        """只允许在服务器端调用推送数据
        """
        assert openfed.is_king()
        if pg is None:
        ...
        # 把数据推送到客户端，rank=1
        federated_c10d.gather_object(
            packages, [None, None], dst=1, group=pg, federated_world=fed_world)

    @classmethod
    def pull(cls, pg: ProcessGroup = None) -> PACKAGES:
        if openfed.is_king():
            return cls.pull_from_queen(pg)
        else:
            return cls.pull_from_king(pg)

    @classmethod
    def push(cls, packages: PACKAGES, pg: ProcessGroup = None) -> None:
        if openfed.is_king():
            return cls.push_to_queen(packages, pg)
        else:
            return cls.push_to_king(packages, pg)
...
def valid_process_group(pg: ProcessGroup):
    """用来判断返回的是不是一个有效的PG。
    """
    if pg is not _NULL_GP and pg in __pg_to_federated_world_mapping:
        return True
    else:
        return False
